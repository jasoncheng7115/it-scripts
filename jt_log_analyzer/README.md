...
- **1.3.1** (2025-05-09): 管線輸入與除錯模式
   - 支援從標準輸入 (stdin) 讀取資料 (`cat file.log | ./jt_log_analyzer.py`)
   - 新增 `-f <檔案路徑>` 明確指定輸入檔案參數
   - 新增 `-q <關鍵字>` 明確指定篩選關鍵字參數
   - 新增 `-d` / `--debug` 除錯模式，提供詳細的檢測資訊
   - 新增 `-e` / `--encoding` 指定檔案編碼選項
   - 新增 `-h` / `--help` 顯示詳細的說明訊息
   - 增強的錯誤處理和故障診斷提示
   - 強化時間戳記偵測功能，支援更多格式
   - 新增退化機制當標準模式無法匹配時
