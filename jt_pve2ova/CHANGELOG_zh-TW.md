...
### 新增
- **轉換前檔案檢查** — 磁碟轉換開始前，腳本會檢查輸出 OVA（或 VMX 模式下的 VMX）是否已存在。若已存在，自動加上 `_N` 後綴（如 `_1`、`_2`）並通知使用者，避免浪費轉換時間。
- **輸出檔名含 ESXi 版本** — 輸出檔案現在包含目標 ESXi 版本（如 `vmname_esxi6.7.ova`、`vmname_esxi8.0.vmx`），方便辨識 OVA 對應的 ESXi 版本。
