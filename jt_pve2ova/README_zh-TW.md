...
- **智慧偵測** – 來源路徑一律透過 `pvesm path` 解析；RBD 會得到完整 librbd URI（真實 pool、`conf`/`mon_host`、`id`、`keyring`），`qemu-img` 可直接開啟
...
3. **磁碟路徑解析**
   * **RBD** → 透過 `pvesm path` 解析為完整 librbd URI，如 `rbd:<pool>/<image>:conf=…:id=<user>:keyring=…`（必要時退回從 `storage.cfg` 自行組出）
...
| RBD 權限不足 / `error connecting`             | 確認 `pvesm path <storage>:<vol>` 可正常解析，且其 `conf=`（如 `/etc/pve/ceph.conf`）與 `keyring=` 檔案可讀 |
