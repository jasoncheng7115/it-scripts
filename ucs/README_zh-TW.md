...
| `jt-ucs-ldap-audit.sh` | **唯讀一致性 / 孤兒稽核** | 找重複 ID、dangling 成員、S4 rejects——完全不寫入 |
...
| 5 | 記錄原始關鍵屬性供比對。 |
| 6 | *（群組）* 檢查每個成員是否還存在；可選擇丟棄 dangling 成員。 |
| 7 | 確認還原；DN 已存在則拒絕匯入。 |
...
**unfold**，因此剝除屬性時不會留下孤兒續行害 `ldapadd` 報難懂錯誤（對 `memberOf`、
...
檢查項目：重複的 `sambaSID` / `uidNumber` / `gidNumber`；dangling `memberUid` 與
`uniqueMember`（成員物件已消失）；主要 `gidNumber` 找不到對應群組的帳號；S4 Connector
rejected 物件（Samba4 AD DC）；以及 LDAP 對 Samba 的物件數量落差（資訊性）。離開碼
`0` = 乾淨、`1` = 有問題、`2` = 設定錯誤（方便搭 cron/監控）。
